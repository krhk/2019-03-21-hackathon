...
class FileSystem {
    ...
    public function upload($file, $dir, $allowedExts = array()) {
        ...
        $uploadPath = './data/storage/' . $dir . basename($fileName);
    ...
    public function get(string $file): string {
        $path = ROOT . "data/storage/$file";

        return $path;
    }

    public function move(string $file, string $path): bool {
        if(!rename("./data/storage" . $file, "./data/storage" . $path))
            return false;
    ...
    public function listFiles(string $dir_name, bool $listDirs = false): array {
        ...
        if(!$listDirs || end($real_path) == "data/storage")
    ...
    public function rename(string $file, string $name): bool {
        if(!rename("./data/storage/" . $file, "./data/storage/" . $name))
            return false;
    ...
    public function newDir(string $dir_name): bool {
        if(!mkdir("./data/storage/" . htmlspecialchars($dir_name), 0777, true))
            return false;
