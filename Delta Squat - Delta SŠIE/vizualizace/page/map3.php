...
<script>
    ...
    fetch('http://khk-api.grimir.cz/regions')
        ...
        .then(function(myJson) {
            myJson.data.forEach(val=>{
                count = val.unemployedCount;
                poptext = "<b>"+ val.addr.address.city+"</b><br>"+count+" nezaměstnaných";
    ...
    function writeLegend(){
        ...
        for(let i = 0; i < values.length; i++){
            document.getElementsByClassName("legend")[0].innerHTML += '<tr class="item"><td class="color" style="background-color: rgb(255, '+colors[values.length-i-1]+', 0);"></td><td class="text">'+values[i]+' nezaměstnaných</td></tr>';
            c++;
        }
        if(c<1){
